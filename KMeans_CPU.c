...
#pragma omp parallel for // Mudança feita para a paralelização
            for (int i = 0; i < k; i++)
            {
                clusters[i].x = 0;
                clusters[i].y = 0;
            ...
            cluster *clusters_private = calloc(n_threads * k, sizeof(cluster));

#pragma omp parallel // Acumular localmente
            {
                int tid = omp_get_thread_num();
                cluster *my_clusters = &clusters_private[tid * k];
#pragma omp for
                for (size_t j = 0; j < size; j++)
                {
                    int t = observations[j].group;
                    my_clusters[t].x += observations[j].x;
                    my_clusters[t].y += observations[j].y;
                    my_clusters[t].count++;
                }
            }
            ...
            for (int t = 0; t < k; t++)
            {
                ...
                {
                    clusters[t].x += clusters_private[tid * k + t].x;
                    clusters[t].y += clusters_private[tid * k + t].y;
                    clusters[t].count += clusters_private[tid * k + t].count;
                }
            }
            free(clusters_private);
            for (int i = 0; i < k; i++)
            {
            ...
            changed = 0;
#pragma omp parallel for reduction(+ : changed) // Mudança feita para a paralelização
            for (size_t j = 0; j < size; j++)
            {
