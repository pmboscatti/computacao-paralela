...
cluster* kMeans(observation observations[], size_t size, int k)
{
    ...
    else if (k < size)
    {
        ...
        do
        {
            ...
            #pragma omp target teams distribute parallel for map(to:clusters[0:k]) map(tofrom:observations[0:size])
            for (size_t j = 0; j < size; j++)
            {
...
static void test()
{
    ...
    // CUDA event timing (somente KMEANS)
    cudaEvent_t start_time, end_time;
    cudaEventCreate(&start_time);
    cudaEventCreate(&end_time);

    cudaEventRecord(start_time, 0);
    clusters = kMeans(observations, size, k);  // AQUI mede o tempo
    cudaEventRecord(end_time, 0);
    cudaEventSynchronize(end_time);

    float tempo_execucao = 0;
    cudaEventElapsedTime(&tempo_execucao, start_time, end_time);
